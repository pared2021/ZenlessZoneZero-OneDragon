...
class HollowZeroMapService:
    ...
    def cal_current_map_by_screen(self, screen: MatLike, screenshot_time: float) -> HollowZeroMap | None:
        """
        根据当前的游戏画面 计算对应的空洞地图
        :param screen: 游戏画面
    ...
    def cal_map_by_screen(self, screen: MatLike, screenshot_time: float) -> HollowZeroMap | None:
        """
        根据游戏画面 计算空洞地图
        会与过去一段时间识别到的地图进行合并
        :param screen: 游戏画面
